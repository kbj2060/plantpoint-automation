...
from logger.custom_logger import custom_logger  # 적합한 로거 임포트
...
class WebSocketHandler:
    def __init__(self, url="ws://localhost:4000", headers={"Content-Type": "application/json", "Client-Type": "automation"}):
        self.url = url
    ...
    def _on_message(self, ws, message):
        custom_logger.info(f"메시지 수신: {message}")
    ...
    def emit_switch(self, event: str, machine: str, status: bool):
        """WebSocket을 통해 메시지 전송"""
        if self.connected:
            message = json.dumps({'event': event, 'data': { machine: status }})
            try:
                self.ws.send(message)
                custom_logger.info(f"메시지 전송: {message}")
